...
async def _post_signed(
    ...
    *,
    ...
    if include_svix:
        sig = _svix_signature(body, secret, delivery_id, timestamp)
        if bad_signature:
            sig = "v1," + base64.b64encode(b"not-the-real-digest").decode()
        headers.update(
            {
                "svix-id": delivery_id,
        ...
        # Pass existing generic HMAC auth while omitting Svix delivery id.
