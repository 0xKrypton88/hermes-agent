...
import { createGatewayOpenEpochHandler, reattachLiveGatewaySessions } from '@/lib/live-session-reattach'
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    configureGatewayRegistry({
      ...
      onOpen: profileGateway => reattachLiveGatewaySessions(profileGateway)
    })

    const handlePrimaryOpenEpoch = createGatewayOpenEpochHandler(() => reattachLiveGatewaySessions(gateway))

    const offState = gateway.onState(st => {
      handlePrimaryOpenEpoch(st)
