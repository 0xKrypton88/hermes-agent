...
class WebhookAdapter(BasePlatformAdapter):
    ...
    def _get_receipt_store(self, profile: str = "default"):
        """Return a profile-keyed webhook receipt store.

        Keep ``_receipt_store`` as the default-profile injection seam used by
        focused tests and older embedders; named profiles never share it.
        """
        profile = (profile or "default").strip() or "default"
        store = getattr(self, "_receipt_store", None)
        if profile == "default" and store is not None:
            return store

        stores = getattr(self, "_receipt_stores", None)
        if stores is None:
            stores = {}
            self._receipt_stores = stores
        if profile not in stores:
            ...
            stores[profile] = WebhookReceiptStore(
                db_path=profile_home / "webhook_receipts.db"
            )
    ...
    async def _handle_linear_issue_go_receipt(
        ...
        *,
        ...
        delivery_id = _header("Linear-Delivery").strip()
        if not delivery_id:
            logger.warning(
                "[webhook] receipt_only linear_issue_go missing Linear-Delivery route=%s",
                route_name,
            )
            return web.json_response(
        ...
        try:
            store = self._get_receipt_store(profile)
            result = await asyncio.to_thread(
                store.record_linear_issue_go,
                delivery_id=delivery_id,
        ...
        if result.status == "conflict":
            logger.warning(
                "[webhook] receipt_only linear_issue_go delivery conflict route=%s delivery=%s",
                route_name,
                delivery_id,
            )
            return web.json_response(
                {"error": "Delivery id conflicts with existing payload"},
                status=409,
            )
